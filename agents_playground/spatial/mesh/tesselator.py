    ...
    def tesselate(self, mesh: MeshLike):
        ...
        for face in mesh.faces:
            ...
            if not is_convex(vertices):
                raise TesselationException(
                    f"A fan cannot be produced from the vertices:\n{vertices}"
                )
...
def is_convex(vertices: tuple[MeshVertexLike, ...]) -> bool:
    ...
    match dim:
        ...
        case _:
            raise TesselationException(
                f"Attempted to tesselate a polygon with a vertex with {dim} dimensions."
            )
...
def is_2d_polygon_convex(vertices: tuple[MeshVertexLike, ...]) -> bool:
    ...
    for current_vert in range(num_verts):
        ...
        vector_a = vector_from_points(
            vert_b.location, vert_a.location
        )  # vert_b -> vert_a
        vector_b = vector_from_points(
            vert_b.location, vert_c.location
        )  # vert_b -> vert_c
...
def is_3d_polygon_convex(vertices: tuple[MeshVertexLike, ...]) -> bool:
    ...
    for current_vert in range(num_verts):
        ...
        v1 = vector_from_points(
            vertices[previous_vert].location, vertices[current_vert].location
        )
        edge_normal = poly_normal.cross(v1)
        v2 = vector_from_points(
            vertices[next_vert].location, vertices[current_vert].location
        )
        alignment_between_next_edge_and_normal: float = edge_normal * v2
        current_direction = math.copysign(
            current_direction, alignment_between_next_edge_and_normal
        )
        if current_vert == 0:
