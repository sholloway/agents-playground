...
from agents_playground.spatial.landscape.landscape_file_characteristics import (
    LandscapeFileCharacteristics,
)
...
class Landscape:
    ...
    def __post_init__(self) -> None:
        ...
        wrap_field_as_maybe(
            self, "file_characteristics", lambda f: LandscapeFileCharacteristics(**f)
        )
...
def cubic_tile_to_vertices(
    tile: Tile, lc: LandscapeCharacteristics
) -> list[Coordinate]:
    """
    ...
    vertex_unit_vectors: tuple[Vector, ...] = TileCubicVerticesPlacement[
        TileCubicPlacement(tile_orientation)
    ]
    ...
    scaled_vertices: list[Coordinate] = [
        Coordinate(
            v.i * lc.tile_width + x_offset,
            v.j * lc.tile_height + y_offset,
            v.k * lc.tile_depth + z_offset,
        )
