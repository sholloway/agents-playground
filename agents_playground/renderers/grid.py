...
def render_grid(**data) -> None:
    ...
    dpg.draw_rectangle(
        pmin=(0, 0), pmax=(grid_width, grid_height), fill=grid_background_color
    )
    ...
    for row in range(rows + 1):
        ...
        line_thickness = (
            grid_line_thickness if row % grid_marker else grid_marker_line_thickness
        )
        dpg.draw_line(
    ...
    for col in range(columns + 1):
        ...
        line_thickness = (
            grid_line_thickness if col % grid_marker else grid_marker_line_thickness
        )
        dpg.draw_line(
