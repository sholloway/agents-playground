...
def update_uniforms(
    device: wgpu.GPUDevice, camera_buffer: wgpu.GPUBuffer, camera: Camera3d
) -> None:
...
class ObjPipeline(WebGpuPipeline):
    ...
    def initialize_pipeline(self, canvas: WgpuWidget) -> None:
        ...
        frame_data: PerFrameData = renderer.prepare(
            device, render_texture_format, mesh, self._camera, model_world_transform
        )
        ...
        self._bound_draw_frame = partial(
            draw_frame, self._camera, canvas, device, renderer, frame_data
        )
