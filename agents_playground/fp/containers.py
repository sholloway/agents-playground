...
class FPList(UserList[A], Applicative, Functor, Tick):
    ...
    def apply(self: FPList[Callable[[B], B]], other: Wrappable[B]) -> "FPList[B]":
        """
...
class FPDict(UserDict[FPDictKey, FPDictValue], Functor, Tick, Wrappable):
    ...
    def map(
        self, func: Callable[[FPDictValue], FPDictKeyNewValue]
    ...
    def wrap(
        self, a_dict: Dict[FPDictKey, FPDictValue]
...
class FPSet(MutableSet[FPSetItem], Applicative, Functor, Tick):
    ...
    def map(self, func: Callable[[FPSetItem], FPNewSetItem]) -> FPSet[FPNewSetItem]:
        """
        Applies a function to all of the values in the dict and
        returns a new FPSet.
        """
    ...
    def wrap(self, a_set: Set[FPSetItem]) -> FPSet[FPSetItem]:
        return FPSet(a_set)

    def unwrap(self) -> Set[FPSetItem]:
        return set(self._data)

    def apply(self: FPSet[Callable[[B], B]], other: Wrappable[B]) -> "FPSet[B]":
        """
