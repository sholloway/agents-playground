...
class AgentRecognitionSystem(SystemWithByproducts):
    def __init__(
        ...
        super().__init__(
            name="recognition_system", byproduct_defs=[], internal_byproduct_defs=[]
        )
    ...
    def _before_subsystems_processed_pre_state_change(
        ...
        """
        sensed_agents: FPList[VisualSensation] = self._filter_visual_sensations(
    ...
    def _filter_visual_sensations(
        self, agent_memory: AgentMemoryModel
    ) -> FPList[VisualSensation]:
        try:
            input_memories: FPList[Memory] = agent_memory[
                self._input_memory_container
            ].unwrap()
            sensed_agents: List[VisualSensation] = [
                sense_memory.unwrap()
                for sense_memory in input_memories
    ...
    def _attempt_recognition(
        ...
        try:
            working_memories: TTLStore[Memory] = agent_memory[
            ...
            for seen_agent in sensed_agents:
                for agent_id in seen_agent.seen:
                    ...
                    seen_distance: float = current_location.find_manhattan_distance(
                        other_agent.position.location
                    )
                    if seen_distance <= DEFAULT_RECOGNITION_THRESHOLD:
                        # The agent recognizes the other agent. Record this in the working memory.
                        working_memories.store(
                            Memory[Tag, None](other_agent.identity.id),
                            DEFAULT_RECOGNITION_TTL,
                        )
        except KeyError:
