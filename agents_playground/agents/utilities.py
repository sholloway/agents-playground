...
def update_agent_in_scene_graph(
    agent: AgentLike, node_ref: Tag, terrain_offset: Size
) -> None:
    """
    ...
    #    the canvas space.
    location_on_grid = agent.position.location.multiply(
        Coordinate(terrain_offset.width, terrain_offset.height)
    )
    ...
    affine_transformation_matrix = (
        translate * shift_from_origin_to_cell * rotate * scale
    )
