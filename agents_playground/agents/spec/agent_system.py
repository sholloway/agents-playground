...
class AgentSystemLike(Protocol):
    ...
    byproducts_store: (
        ByproductStore  # The collection of byproducts this system produces.
    )
    ...
    def register_system(self, subsystem: AgentSystemLike) -> Self:
        """
        Add a subsystem.
        ...
        self.byproducts_store.register_subsystem_byproducts(
            self.name, subsystem.name, subsystem.byproducts_definitions
        )
        return self
    ...
    def process(
        ...
        self._before_subsystems_processed(
            characteristics, agent_phase, parent_byproducts, other_agents
        )
    ...
    def _before_subsystems_processed(
        ...
        match agent_phase:
            case agent_phase.PRE_STATE_CHANGE:
                self._before_subsystems_processed_pre_state_change(
                    characteristics, parent_byproducts, other_agents
                )
            case agent_phase.POST_STATE_CHANGE:
                self._before_subsystems_processed_post_state_change(
                    characteristics, parent_byproducts, other_agents
                )
            case _:
                error_msg = f"""
    ...
    def _after_subsystems_processed(
        ...
        match agent_phase:
            case agent_phase.PRE_STATE_CHANGE:
                self._after_subsystems_processed_pre_state_change(
                    characteristics, parent_byproducts, other_agents
                )
            case agent_phase.POST_STATE_CHANGE:
                self._after_subsystems_processed_post_state_change(
                    characteristics, parent_byproducts, other_agents
                )
            case _:
                error_msg = f"""
    ...
    def _collect_byproducts_from_subsystems(self) -> None:
        ...
        for subsystem in self.subsystems.__dict__.values():
            for byproduct_def in subsystem.byproducts_definitions:
                byproduct = subsystem.byproducts_store.byproducts.get(
                    byproduct_def.name, []
                )
