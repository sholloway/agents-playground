...
class AgentLike(FrameTick, Protocol):
  ...
  """
    ...
    def face(self, direction: Vector, cell_size: Size) -> None:
        ...
        self.physicality.frustum.update(
            self.position.location, self.position.facing, cell_size
        )
    ...
    def _pre_state_change_process_subsystems(
        self, characteristics: AgentCharacteristics, other_agents: Dict[Tag, AgentLike]
    ) -> None:
        """Internal hook responsible for processing the subsystems before the agent's status changes."""
        self.internal_systems.process(
            characteristics, AgentLifeCyclePhase.PRE_STATE_CHANGE, other_agents
        )

    def _post_state_change_process_subsystems(
        self, characteristics: AgentCharacteristics, other_agents: Dict[Tag, AgentLike]
    ) -> None:
        """Internal hook responsible for processing the subsystems before the agents status changes."""
        self.internal_systems.process(
            characteristics, AgentLifeCyclePhase.POST_STATE_CHANGE, other_agents
        )
