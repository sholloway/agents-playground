...
class ByproductStore:
    ...
    def register_system_byproducts(
        self, system_name: str, byproduct_defs: List[ByproductDefinition]
    ) -> None:
        """
        ...
        for byproduct_def in byproduct_defs:
            if not byproduct_def.name in self.byproducts:
                ...
                self._registered_byproducts[byproduct_def.name].type
                != byproduct_def.type
            ):
                error_msg = f"""
    ...
    def clear(self) -> None:
        ...
        """
        consume(map(lambda byproduct: byproduct.clear(), self._byproducts.values()))
