...
from agents_playground.agents.spec.agent_state_spec import (
...
class DefaultAgentState(AgentStateLike):
    ...
    def transition_to_next_action(
        self, agent_characteristics: AgentCharacteristics
    ) -> None:
        self.last_action_state = self.current_action_state
        self.current_action_state = self.action_selector.next_action(
            agent_characteristics, self.current_action_state
        )
