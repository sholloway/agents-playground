...
class Memory(Monad, Hashable, Generic[MemoryValue, MemoryMetadata]):
    ...
    def __init__(
        self, core_memory: MemoryValue, memory_metadata: MemoryMetadata | None = None
    ) -> None:
        """
    ...
    def wrap(self, core_memory: MemoryValue) -> "Memory[MemoryValue, Nothing]":
        return Memory(core_memory=core_memory, memory_metadata=Nothing())
    ...
    def metadata(self) -> Maybe[MemoryMetadata]:
        return (
            ...
            else Something(self._memory_metadata)
        )

    def bind(
        self, next_func: Callable[[MemoryValue], Bindable[MemoryValue]]
