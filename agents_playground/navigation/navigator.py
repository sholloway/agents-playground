...
class Navigator:
  ...
  def find_route(
    ...
    starting_point.cost_to_target = starting_point.point.find_manhattan_distance(desired_location)
    ...
    while len(possible_steps) > 0:
      ...
      if current_location.point == desired_location:
        ...
        current_junction: Junction = nav_mesh.get_junction_by_location(current_location.point)
        ...
        for neighbor_location_toml_id in current_junction.connects_to:
          ...
          neighbor_junction: Junction = nav_mesh.get_junction_by_toml_id(neighbor_location_toml_id)
          neighbor: Waypoint = Waypoint(neighbor_junction.location, current_location)
          ...
          cost_to_add_step_to_path: NavigationCost = current_location.total_cost() + current_location.point.find_manhattan_distance(neighbor.point)
          neighbor.cost_from_start = cost_to_add_step_to_path
          neighbor.cost_to_target = neighbor.point.find_manhattan_distance(desired_location)
          ...
          if (neighbor.point in visited_locations) and (cost_to_add_step_to_path < starting_point.point.find_manhattan_distance(neighbor.point)) :
          ...
          if (neighbor in possible_steps) and (cost_to_add_step_to_path < starting_point.point.find_manhattan_distance(neighbor.point)):
