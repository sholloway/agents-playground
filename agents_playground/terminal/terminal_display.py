...
class TerminalDisplay:
    def __init__(
        self, terminal_layer_id: Tag, display_id: Tag, context: SimulationContext
    ) -> None:
    ...
    def refresh(self, screen_buffer: TerminalBuffer) -> None:
        ...
        dpg.delete_item(item=self._terminal_layer_id, children_only=True, slot=2)
        ...
        for buffer_line in screen_buffer.scroll_back_buffer:
            vertical_offset = (
                TERM_DISPLAY_INITIAL_TOP_OFFSET
        ...
        for prompt_line_index, prompt_line in enumerate(screen_buffer.active_prompt):
            current_line_vertical_offset = (
                ...
                + (
                    (current_line + prompt_line_index)
                    * TERM_DISPLAY_VERTICAL_LINE_SPACE
                )
            )
            dpg.draw_text(
                parent=self._terminal_layer_id,
        ...
        block_vertical_offset = (
            active_prompt_first_line_vo
            + screen_buffer.cursor_vertical_location
            * (TERM_DISPLAY_LINE_HEIGHT + TERM_DISPLAY_VERTICAL_LINE_SPACE)
        )
        dpg.draw_text(
            parent=self._terminal_layer_id,
            pos=(
                TERM_DISPLAY_LEFT_OFFSET
                + DEFAULT_FONT_SIZE
