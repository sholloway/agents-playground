...
def find_centered_window_position(
    parent_width: int, parent_height: int, child_width: int, child_height: int
) -> Tuple[int, int]:
    center = (
    ...
    with dpg.tree_node(label=label):
        with dpg.table(
            ...
            dpg.add_table_column(
                label="Value", width_stretch=True, init_width_or_weight=0.0
            )
...
def add_table_of_namespaces(
    label: str, columns: List[str], rows: List[SimpleNamespace]
) -> None:
    with dpg.tree_node(label=label):
        with dpg.table(
            header_row=True,
