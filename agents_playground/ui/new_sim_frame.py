...
from agents_playground.projects.new_simulation_builder import (
    NewSimulationBuilder,
    NewSimulationBuilderException,
)
from agents_playground.projects.simulation_template_options import (
    SimulationTemplateOptions,
)
from agents_playground.ui.validators.cannot_be_empty_validator import (
    CannotBeEmptyValidator,
)
from agents_playground.ui.validators.directory_must_exist_validator import (
...
NEW_SIM_NAME_TOOLTIP = (
    "Assign a unique name for the simulation.\nThis is the name of the simulation file."
)
...
NEW_SIM_NAME_FORMAT_ERROR = (
    "Only the lower case characters a-1 and the _ character are allowed."
)
...
class NewSimFrame(wx.Frame):
    ...
    def _build_sim_description_components(self) -> None:
        self._sim_description_label = wx.StaticText(
            self._panel, label="Simulation Description"
        )
    ...
    def _build_sim_name_components(self) -> None:
        ...
        # a single line.
        self._sim_name_input.SetBackgroundColour(
            self._sim_description_input.GetBackgroundColour()
        )
    ...
    def _layout_components(self) -> None:
        ...
        grid_sizer.Add(
            ...
            border=NEW_SIM_GRID_BOARDER,
        )
        grid_sizer.Add(
            self._sim_name_label,
            pos=(2, 1),
            span=(1, 1),
            flag=wx.ALL,
            border=NEW_SIM_GRID_BOARDER,
        ...
        grid_sizer.Add(
            self._scene_distance_uom_label,
            pos=(7, 1),
            span=(1, 1),
            flag=wx.ALL,
            border=NEW_SIM_GRID_BOARDER,
        ...
        grid_sizer.Add(
            self._scene_distance_uom_choice,
            pos=(7, 2),
            span=(1, 1),
            flag=wx.EXPAND | wx.ALL,
            border=NEW_SIM_GRID_BOARDER,
        ...
        #
        grid_sizer.Add(
            self._landscape_uom_system_label,
            pos=(8, 1),
            span=(1, 1),
            flag=wx.ALL,
            border=NEW_SIM_GRID_BOARDER,
        ...
        grid_sizer.Add(
            self._landscape_uom_system_choice,
            pos=(8, 2),
            span=(1, 1),
            flag=wx.EXPAND | wx.ALL,
            border=NEW_SIM_GRID_BOARDER,
        ...
        grid_sizer.Add(
            self._landscape_tile_size_uom_label,
            pos=(9, 1),
            span=(1, 1),
            flag=wx.ALL,
            border=NEW_SIM_GRID_BOARDER,
        ...
        grid_sizer.Add(
            self._landscape_tile_size_uom_choice,
            pos=(9, 2),
            span=(1, 1),
            flag=wx.EXPAND | wx.ALL,
            border=NEW_SIM_GRID_BOARDER,
    ...
    def _build_sim_uom_components(self) -> None:
        ...
        self._scene_uom_system_label = wx.StaticText(
            self._panel, label="Unit of Measure System"
        )
        self._scene_uom_system_choice = wx.Choice(
            self._panel, choices=[e.value for e in SceneUOMOptions]
        )
        self._scene_uom_system_choice.Bind(
            wx.EVT_CHOICE, self._handle_scene_uom_selected
        )
        self._scene_uom_system_choice.SetToolTip(NEW_SIM_SIM_UOM_SYSTEM)

        # Scene Distance UOM
        self._scene_distance_uom_label = wx.StaticText(
            self._panel, label="Distance UOM"
        )
        self._scene_distance_uom_choice = wx.Choice(
            self._panel, choices=METRIC_DISTANCE_OPTIONS
        )
        self._scene_distance_uom_choice.SetToolTip(NEW_SIM_DISTANCE_UOM)

    def _build_landscape_uom_components(self) -> None:
        # Simulations UOM System
        self._landscape_uom_system_label = wx.StaticText(
            self._panel, label="Landscape Unit of Measure System"
        )
        self._landscape_uom_system_choice = wx.Choice(
            self._panel, choices=[e.value for e in SceneUOMOptions]
        )
        self._landscape_uom_system_choice.Bind(
            wx.EVT_CHOICE, self._handle_landscape_uom_selected
        )
        self._landscape_uom_system_choice.SetToolTip(NEW_SIM_LANDSCAPE_UOM_SYSTEM)

        # Scene Distance UOM
        self._landscape_tile_size_uom_label = wx.StaticText(
            self._panel, label="Tile Size UOM"
        )
        self._landscape_tile_size_uom_choice = wx.Choice(
            self._panel, choices=METRIC_DISTANCE_OPTIONS
        )
        ...
        if self.Validate():
            try:
                options = SimulationTemplateOptions(
                    ...
                    scene_uom_system=self._scene_uom_system_choice.GetStringSelection().replace(
                        CHAR_SPACE, CHAR_UNDERSCORE
                    ),
                    ...
                    landscape_uom_system=self._landscape_uom_system_choice.GetStringSelection().replace(
                        CHAR_SPACE, CHAR_UNDERSCORE
                    ),
