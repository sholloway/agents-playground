...
class ContextViewerWindow:
  def launch(self, context: SimulationContext, height: int, inspect_agent_callback: Callable) -> None:
      ...
      with dpg.tree_node(label = 'Scene'):
        ...
        if len(context.scene.nav_mesh._junctions) > 0:
          with dpg.tree_node(label = 'Navigation Mesh'):
            junction_rows: List[SimpleNamespace] = list(context.scene.nav_mesh.junctions())
            add_table_of_namespaces(
              label = 'Junctions', 
