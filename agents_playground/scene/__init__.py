...
from agents_playground.agents.default.default_agent_movement_attributes import (
    DefaultAgentMovementAttributes,
)
from agents_playground.agents.default.default_agent_physicality import (
...
class Scene(Tick):
    ...
    def _init_landscape_transformation(self) -> None:
        if isinstance(self.landscape_transformation, dict):
            self.landscape_transformation = TransformationConfiguration(
                **self.landscape_transformation
            )
    ...
    def _init_agents(self) -> None:
        ...
        if (
            ...
            or not isinstance(self.agents[0], dict)
        ):
            return
...
class SceneAgentBuilder:
    ...
    def build(self, agent_attributes: dict[str, Any]) -> AgentLike:
        try:
            ...
            agent_identity: AgentIdentityLike = self._agent_identity(
                agent_attributes
            )  # TODO: Clean up DefaultAgentIdentity class after removing DearPyGUI.
    ...
    def _agent_position(self, agent_attributes: dict[str, Any]) -> AgentPositionLike:
        ...
        agent_position = DefaultAgentPosition(
            facing=vector(
                1, 0, 0
            ),  # For the moment make them all face parallel to the X-axis.
    ...
    def _agent_state(self, agent_def: AgentDefinition) -> AgentStateLike:
        ...
        if agent_def.agent_state_model.is_something():
            agent_state_model: FsmAgentStateModel = cast(
                FsmAgentStateModel, agent_def.agent_state_model.unwrap()
            )
