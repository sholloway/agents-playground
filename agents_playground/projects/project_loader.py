    ...
    def validate(self, module_name: str, module_path: str) -> bool:
        if not module_name.islower() or not module_name.isidentifier():
            raise ProjectLoaderError(
                f"The module name {module_name} is not formatted correctly. Modules must be all lowercase. Underscores are allowed."
            )
        return True
...
class InitFileExist(ProjectValidator):
    def validate(self, module_name: str, module_path: str) -> bool:
        if not os.path.exists(os.path.join(module_path, "__init__.py")):
            raise ProjectLoaderError(
                f"The module does not contain an __init__.py file."
            )
        return True
...
class ProjectLoader:
    ...
    def _load_project(self, module_name: str, project_path: str) -> None:
        ...
        spec = get_or_raise(
            spec_from_loader(loader.name, loader), Exception(SPEC_FAILED_ERROR_MSG)
        )
        module = get_or_raise(
            module_from_spec(spec), Exception(MOD_FAILED_FROM_SPEC_ERROR_MSG)
        )
