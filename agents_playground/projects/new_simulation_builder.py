...
def populate_template(
    path_to_template: Path, path_to_target: Path, template_inputs: dict[str, Any]
) -> None:
...
class NewSimulationBuilder:
    ...
    def _prevent_over_writing_existing_directories(
        self, simulation_options: SimulationTemplateOptions
    ) -> None:
        new_sim_dir = os.path.join(
            simulation_options.parent_directory, simulation_options.simulation_name
        )
    ...
    def _copy_template_directory(
        ...
        try:
            ...
            template_dir = os.path.join(
                Path.cwd(), "agents_playground/templates/new_sim"
            )
            shutil.copytree(template_dir, new_sim_dir)
        except Exception as e:
            raise NewSimulationBuilderException(
                "Failed to copy the template directory for the new simulation."
            ) from e

    def _rename_pkg_directory(
        self, simulation_options: SimulationTemplateOptions
    ) -> None:
        try:
            package_dir = os.path.join(
                simulation_options.parent_directory,
                simulation_options.simulation_name,
                "project_pkg",
            )
            project_pkg_dir = os.path.join(
                simulation_options.parent_directory,
                simulation_options.simulation_name,
                simulation_options.simulation_name,
            )
            ...
            raise NewSimulationBuilderException(
                "Failed to rename the package directory for the new simulation."
            ) from e
    ...
    def _generate_project_files(
        ...
        try:
            ...
            template_inputs["default_template_path"] = (
                "agents_playground/templates/base_sim_files"
            )
            ...
            for template in TEMPLATES:
                hydrated_template_location: str = Template(
                    template.template_location
                ).substitute(template_inputs)
                hydrated_template_name: str = Template(
                    template.template_name
                ).substitute(template_inputs)
                hydrated_target: str = Template(template.target_location).substitute(
                    template_inputs
                )
                template_path: Path = Path(
                    os.path.join(
                        Path.cwd(), hydrated_template_location, hydrated_template_name
                    )
                )
                target_path: Path = Path(
                    os.path.join(
                        new_project_dir, hydrated_target, hydrated_template_name
                    )
                )
                populate_template(template_path, target_path, template_inputs)
        except Exception as e:
            raise NewSimulationBuilderException(
                "Failed to generate the project files for the new simulation."
            ) from e
    ...
    def _copy_wheel(self, simulation_options: SimulationTemplateOptions) -> None:
        ...
        try:
            ...
            destination_path: Path = Path(
                os.path.join(new_project_dir, "libs", engine_pkg)
            )
            shutil.copyfile(wheel_path, destination_path)
        except Exception as e:
            raise NewSimulationBuilderException(
                "Failed to copy the engine's wheel while trying to set up the new simulation."
            ) from e
