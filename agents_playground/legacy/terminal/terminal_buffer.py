...
class TerminalBuffer:
    ...
    def __init__(self) -> None:
        self._scroll_back_buffer: Deque[TerminalBufferContent] = deque(
            [], maxlen=SCROLL_BACK_BUFFER_MAX_LENGTH
        )
        self._history_buffer: Deque[TerminalBufferContent] = deque(
            [], maxlen=HISTORY_BUFFER_MAX_LENGTH
        )
        """
    ...
    def _remember(
        self, output: TerminalBufferContent | List[TerminalBufferContent]
    ) -> None:
    ...
    def add_new_line(self) -> None:
        ...
        self._active_prompt.insert(
            cast(int, self._cursor_vertical_position.value() + 1), ""
        )

        # 2. If there is text to the RIGHT of the cursor, move that to the next line.
        current_line = self.active_prompt[
            cast(int, self._cursor_vertical_position.value())
        ]
        cursor_loc = cast(int, self._cursor_horizontal_position.value())
        if cursor_loc < len(current_line) - 1:
            self.active_prompt[
                cast(int, self._cursor_vertical_position.value() + 1)
            ] = current_line[cursor_loc:]
            self.active_prompt[cast(int, self._cursor_vertical_position.value())] = (
                current_line[:cursor_loc]
            )
    ...
    def remove(self, num_chars_to_remove: int) -> None:
        ...
        if prompt_line > 0 and (cursor_loc - num_chars_to_remove) < 0:
    ...
    def shift_prompt_right(self, amount: int = 1) -> None:
        ...
        len_current_line = len(
            self._active_prompt[cast(int, self._cursor_vertical_position.value())]
        )
        for _ in range(amount):
