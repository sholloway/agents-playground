...
class TerminalInterpreter(Interpreter, ExprVisitor[Any], StmtVisitor[None]):
    def __init__(self, buffer: TerminalBuffer, display: TerminalDisplay) -> None:
        ...
        self._scoped_environment: Environment = (
            self._globals
        )  # Tracks the currently scoped environment.
        ...
        self._interpreter_mode: InterpreterMode
        # Assigned during interpret() invocation
    ...
    def interpret(
        self, statements: List[Stmt], mode: InterpreterMode = InterpreterMode.COMMAND
    ) -> None:
        ...
        try:
            ...
            raise InterpreterRuntimeError(
                cf.token, "A control flow signal was not handled."
            ) from cf
    ...
    def visit_expression_stmt(self, stmt: Expression) -> None:
        ...
        if self._interpreter_mode == InterpreterMode.COMMAND:
            self._terminal_buffer.append_output(
                TerminalBufferUnformattedText(f"{str(result)}"), remember=False
            )
    ...
    def visit_print_stmt(self, stmt: Print) -> None:
        ...
        self._terminal_buffer.append_output(
            TerminalBufferUnformattedText(f"{chr(0xE285)} {str(value)}"), remember=False
        )
        self._terminal_display.refresh(self._terminal_buffer)
    ...
    def visit_history_stmt(self, stmt: History) -> None:
        self._terminal_buffer.append_output(
            self._terminal_buffer.history(), remember=False
        )
        self._terminal_display.refresh(self._terminal_buffer)
        ...
        match expression.operator.type:
            ...
            case TokenType.PLUS:  # Plus can both add numbers and concatenate strings.
                if isinstance(left, Number) and isinstance(right, Number):
                    ...
                    raise InterpreterRuntimeError(
                        expression.operator,
                        "Operands must be a combination of numbers and strings.",
                    )
    ...
    def visit_call_expr(self, expression: Call) -> Any:
        ...
        if len(arguments) != function.arity():
            raise InterpreterRuntimeError(
                expression.paren,
                f"Expected {function.arity()} arguments but there was only {len(arguments)}.",
            )
