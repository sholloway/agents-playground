...
class AgentShell:
    def __init__(self, buffer: TerminalBuffer, display: TerminalDisplay) -> None:
        ...
        self._lexer = Lexer()
        self._interpreter = TerminalInterpreter(
            self._terminal_buffer, self._terminal_display
        )
    ...
    def handle_parser_errors(self, line_num: int, messages: List[str]) -> None:
        ...
        self._terminal_buffer.append_output(
            TerminalBufferErrorMessage(generic_error_msg), remember=False
        )
        self._terminal_buffer.append_output(
            TerminalBufferErrorMessage(f"Error on line {line_num}"), remember=False
        )
        for msg in messages:
            self._terminal_buffer.append_output(
                TerminalBufferErrorMessage(msg), remember=False
            )
    ...
    def run(self, terminal_mode: AgentTerminalMode) -> None:
        try:
            ...
            history: List[TerminalBufferContent] = list(
                map(
                    lambda line: TerminalBufferUserInput(line),
                    self._terminal_buffer.active_prompt,
                )
            )
            ...
            if parser._encountered_error:
                self._terminal_buffer.append_output(
                    TerminalBufferErrorMessage("Encountered a parser error.")
                )
            elif statements is None:
                self._terminal_buffer.append_output(
                    TerminalBufferErrorMessage("Parser returned NoneType.")
                )
            else:
                ...
                interpreter_mode: InterpreterMode = TERMINAL_TO_INTERPRETER_MODE[
                    terminal_mode
                ]
            ...
            error_msg = pe.args[0]
            self._terminal_buffer.append_output(
                TerminalBufferErrorMessage(f"{generic_error_msg}"), remember=False
            )
            self._terminal_buffer.append_output(
                TerminalBufferErrorMessage(f"{specific_info}"), remember=False
            )
            self._terminal_buffer.append_output(
                TerminalBufferErrorMessage(f"{error_msg}"), remember=False
            )
            self._terminal_display.refresh(self._terminal_buffer)
