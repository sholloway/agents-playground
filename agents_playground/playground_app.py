...
class PlaygroundApp(Observer):
  ...
  def _setup_menu_bar(self):
    ...
    with dpg.viewport_menu_bar():
      with dpg.menu(label="Simulations"):
        dpg.add_menu_item(label="Pulsing Circle", callback=self._launch_simulation, tag=self.menu_items['sims']['pulsing_circle_sim'])
  ...
  def _launch_simulation(self, sender, item_data, user_data):
    ...
    elif sender is self.menu_items['sims']['pulsing_circle_sim']:
      self._active_simulation = PulsingCircleSim()
    
    if self._active_simulation is not None:
      self._active_simulation.set_primary_window(self.primary_window_ref)
