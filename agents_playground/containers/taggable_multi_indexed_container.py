...
class TaggableMultiIndexedContainerError(Exception):
    def __init__(self, *args: object) -> None:
        super().__init__(*args)


class TaggableMultiIndexedContainer(
    MultiIndexedContainerLike[D], TaggableContainerLike[D]
):
    """
    ...
    def __init__(self) -> None:
        ...
        # Store tags/labels of items. This enables looking up all items that have an associated tag.
    ...
    def track(self, item: D, id: int, name: str) -> None:
        ...
        if id in self._id_index:
            raise TaggableMultiIndexedContainerError(
                f"The Item {id} is already being tracked."
            )
    ...
    def tagged_items(self, tag_or_tags: str | Sequence[str]) -> tuple[D]:
        ...
        if len(item_indexes) < 1:
            # No items where found so return an empty tuple.
            return tuple([])
    ...
    def tag(self, key: int | str, tag: str) -> None:
        ...
        match key:
            ...
            case _:
                raise TaggableMultiIndexedContainerError("Unhandled key type.")
    ...
    def __getitem__(self, key: int | str) -> D:
        ...
        if item is None:
            raise TaggableMultiIndexedContainerError(
                "No item with key {key} was found."
            )
        return item
