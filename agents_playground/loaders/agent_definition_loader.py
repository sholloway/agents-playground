...
class FsmAgentStateModel:
  ...
  def __post_init__(self) -> None:
    ...
    if isinstance(self.initial_agent_state, str):
      ...
      self.initial_agent_state = NamedAgentActionState(name=self.initial_agent_state)

    if isinstance(self.state_transition_map, list):
      # Convert list of dicts to an AgentActionSelector instance.
      self.state_transition_map = self._build_agent_action_selector(self.state_transition_map)
  ...
  def _build_agent_action_selector(self, transition_rules: list[dict[str, Any]]) -> AgentActionSelector:
    ...
    rule_set: AgentActionStateRulesSet = self._build_state_transition_rules_set(transition_rules)
      
    # 3. Construct the map of rule evaluation sets for each state.
    state_map: dict[AgentActionStateLike, AgentActionStateRulesSet] = self._build_map_of_state_transition_rules(rule_set)

    return FuzzyAgentActionSelector(state_map=state_map)
  
  def _build_state_transition_rules_set(self, transition_rules: list[dict[str, Any]]) -> AgentActionStateRulesSet:
    ...
    return DefaultAgentActionStateRulesSet(
      rules=rules_list, 
      default_state=self.initial_agent_state
    )
  
  def _build_map_of_state_transition_rules(self, rule_set: AgentActionStateRulesSet) -> dict[AgentActionStateLike, AgentActionStateRulesSet]:
    ...
    agent_states_with_rules: set[AgentActionStateLike] = set([rule.state for rule in rule_set.rules])
...
class AgentDefinition:
  ...
  def __post_init__(self) -> None:
    ...
    wrap_field_as_maybe(self, 'agent_state_model', lambda f: FsmAgentStateModel(**f))
