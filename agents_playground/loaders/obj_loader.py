...
class ObjVertexLineParser:
    def parse(
        self, obj: Obj, tokens: List[str], line: str, line_num: int, strict=False
    ) -> None:
    ...
    def _raise_error(self, line_num: int, line: str) -> None:
        raise ObjParserMalformedVertexError(
            f"Line: {line_num} - Vertex definition must be of the form\nv x y z [w]\nFound: {line}"
        )
...
class ObjTextureCoordinateLineParser:
    def parse(
        ...
        try:
            ...
            if strict:
                if (
                    not is_in_unit_interval(u)
                    or not is_in_unit_interval(v)
                    or not is_in_unit_interval(w)
                ):
                    self._raise_error(line_num, line)
    ...
    def _raise_error(self, line_num: int, line: str) -> None:
        raise ObjParserMalformedTextureCoordinateError(
            f"Line: {line_num} - Texture Coordinate definition must be of the form\nvt u [v] [w] where u,v,w are in the unit interval.\nFound: {line}"
        )
...
class ObjVertexNormalLineParser:
    ...
    def _raise_error(self, line_num: int, line: str) -> None:
        raise ObjParserMalformedVertexNormalError(
            f"Line: {line_num} - Vertex normal definition must be of the form\nvn x y z\nFound: {line}"
        )
...
class ObjPolygonLineParser:
    ...
    def _raise_error(self, line_num: int, line: str) -> None:
        raise ObjParserMalformedPolygonError(
            f"Line: {line_num}\n{POLYGON_DEF_EXAMPLE}\nFound: {line}"
        )
