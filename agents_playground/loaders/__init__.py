...
def str_to_datetime(timestamp: str) -> DateTime:
    ...
    return dt.datetime.strptime(timestamp, TIMESTAMP_FORMAT).replace(
...
class ValidateSchemaExists(JSONFileLoaderStep):
    def process(
        ...
        if not os.path.exists(os.path.join(Path.cwd(), schema_path)):
            raise JSONFileLoaderStepException(
                f"Could not find the schema {schema_path}."
            )
        return True
...
class ValidateJSONFileExists(JSONFileLoaderStep):
    ...
    def process(
        ...
        if file_found:
            ...
            raise JSONFileLoaderStepException(
                f"Could not find the JSON file at {file_path} or in {search_directories}."
            )
...
class LoadJSONIntoMemory(JSONFileLoaderStep):
    def process(
        ...
        with open(
            file=context["json_file_path"], mode="r", encoding="utf-8"
        ) as filereader:
...
class ValidateSchema(JSONFileLoaderStep):
    def process(
        ...
        if schema_content is not None:
            ...
            raise JSONFileLoaderStepException(
                f"The schema must be loaded into the context before attempting to validate it."
            )
        return True
...
class ValidateJSONWithSchema(JSONFileLoaderStep):
    def process(
        ...
        js.validate(instance=context["json_content"], schema=context["schema_content"])
        return True
...
class JSONFileLoader:
    ...
    def load(
        ...
        return all(
            [
                r.process(context, schema_path, file_path, search_directories)
