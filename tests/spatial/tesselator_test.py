...
from agents_playground.spatial.mesh.tesselator import (
    FanTesselator,
    Tesselator,
    is_convex,
)
...
class TestTesselator:
    ...
    def test_is_convex_2d(
        self, polygon_a: list[Coordinate], polygon_b: list[Coordinate]
    ) -> None:
        assert is_convex(
            [MeshVertex(coord, index) for index, coord in enumerate(polygon_a)]
        )
        assert not is_convex(
            [MeshVertex(coord, index) for index, coord in enumerate(polygon_b)]
        )

    def test_is_convex_3d(
        self, polygon_aa: list[Coordinate], polygon_bb: list[Coordinate]
    ) -> None:
        assert is_convex(
            [MeshVertex(coord, index) for index, coord in enumerate(polygon_aa)]
        )
        assert not is_convex(
            [MeshVertex(coord, index) for index, coord in enumerate(polygon_bb)]
        )
