...
class TestSceneBuilder:
    ...
    def test_build_agents(self, mocker: MockFixture) -> None:
        ...
        scene_data = SimpleNamespace(
            scene=SimpleNamespace(cell_size=[1, 2], agents=agents)
        )
    ...
    def test_building_linear_paths(self, mocker: MockFixture) -> None:
        ...
        linear_paths = [
            SimpleNamespace(
                ...
                steps=[17, 12, 3, 4],
                renderer="fake_line_segment_renderer",
                closed=True,
            ),
            SimpleNamespace(
                id=42,
        ...
        scene_data = SimpleNamespace(
            scene=SimpleNamespace(
                cell_size=[1, 2], paths=SimpleNamespace(linear=linear_paths)
            )
        )
        ...
        assert (
            scene.paths[path_a_id]._renderer == render_map["fake_line_segment_renderer"]
        )
    ...
    def test_building_circular_paths(self, mocker: MockFixture) -> None:
        ...
        circular_paths = [
            SimpleNamespace(
                id=14, center=[40, 5], radius=4, renderer="circular_path_renderer"
            ),
            SimpleNamespace(
                id=32, center=[40, 5], radius=1, renderer="circular_path_renderer"
            ),
        ]

        scene_data = SimpleNamespace(
            scene=SimpleNamespace(
                cell_size=[1, 2], paths=SimpleNamespace(circular=circular_paths)
            )
        )
        scene: Scene = sb.build(scene_data)
    ...
    def test_scheduling_tasks(self, mocker: MockFixture) -> None:
        spy_id_generator = mocker.spy(dpg, "generate_uuid")
        task_map = {"agent_traverse_linear_path": mocker.Mock()}
        ...
        scene_data = SimpleNamespace(
            scene=SimpleNamespace(cell_size=[1, 2], schedule=schedule)
        )
        scene: Scene = sb.build(scene_data)
    ...
    def test_building_entities(self, mocker: MockFixture) -> None:
        ...
        scene_data = SimpleNamespace(
            scene=SimpleNamespace(
                cell_size=[1, 2], entities=SimpleNamespace(circles=circles)
            )
        )
        scene: Scene = sb.build(scene_data)
