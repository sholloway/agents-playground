...
def assert_token(token, type, lexeme, literal, line) -> None:
    assert (
        token.type == type
    ), f"Wrong token type found. Expected {type} had {token.type}"
    assert (
        token.lexeme == lexeme
    ), f"Wrong token lexeme found. Expected {lexeme} had {token.lexeme}"
    assert (
        token.literal == literal
    ), f"Wrong token literal found. Expected {literal} had {token.literal}"
    assert (
        token.line == line
    ), f"Wrong token line found. Expected {line} had {token.line}"
...
class TestLexer:
    ...
    def test_scanning_string_literals(self) -> None:
        ...
        assert not self._lexer.errors_detected
        assert_token(
            more_tokens[0], TokenType.STRING, "'hello world'", "hello world", 0
        )
    ...
    def test_scanning_reserved_words(self) -> None:
        tokens: List[Token] = self._lexer.scan(
            "var if else and or while for True False"
        )
        assert len(tokens) == 10, "Expected 10 tokens scanned. 9 + EOF"
        assert not self._lexer.errors_detected
    ...
    def test_scanning_history_command(self) -> None:
        tokens: List[Token] = self._lexer.scan("history;")
        assert len(tokens) == 3, "Expected 3 tokens scanned. 2 + EOF"
        assert not self._lexer.errors_detected
        assert_token(tokens[0], TokenType.HISTORY, "history", None, 0)
        assert_token(tokens[1], TokenType.SEMICOLON, ";", None, 0)
        assert_token(tokens[2], TokenType.EOF, "", None, 0)
