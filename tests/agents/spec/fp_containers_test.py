...
from agents_playground.fp.containers import (
    FPDict,
    FPList,
    FPSet,
    FPStack,
...
class TestFPList:
    ...
    def test_it_can_apply(self) -> None:
        ...
        operations = FPList([lambda i: i * i, lambda i: i * 2, lambda i: i + 1])
...
class TestFPDict:
    def test_behaves_like_a_dict(self) -> None:
        ...
        assert FPDict({"c": 789}, a=123, b=456) == FPDict(
            {"a": 123, "b": 456, "c": 789}
        )
...
class TestFPSet:
    ...
    def test_it_can_apply(self) -> None:
        # Apply should work on wrappables.
        operations = FPSet([lambda i: i * i, lambda i: i * 2, lambda i: i + 1])
