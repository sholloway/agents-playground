...
from typing import AbstractSet, Any, Iterable, Iterator, Self, SupportsIndex, cast, Callable, Generic, Protocol, TypeVar, Collection, Hashable
...
# TODO: This is probably the contract for mutable set...
class SetWithTick(Tick, Protocol[T]):
  ...
  def symmetric_difference(self, __s: Iterable[T]) -> set[T]: ...
  def symmetric_difference_update(self, __s: Iterable[T]) -> None: ...
  def union(self, *s: Iterable[S]) -> set[T | S]: ...
  ...
  def __or__(self, __value: AbstractSet[S]) -> set[T | S]: ...
  def __ior__(self, __value: AbstractSet[T]) -> Self: ...  
  def __sub__(self, __value: AbstractSet[T | None]) -> set[T]: ...
  def __isub__(self, __value: AbstractSet[object]) -> Self: ...
  def __xor__(self, __value: AbstractSet[S]) -> set[T | S]: ...
  def __ixor__(self, __value: AbstractSet[T]) -> Self: ...  
...
# Expand this to support, MutableSet, Mapping, MutableMapping, Stack, TTLStorage
