...
class TestProjectSpecificAgents:
    ...
    def test_changing_to_next_state(self, mocker: MockerFixture) -> None:
        ...
        assert agent.internal_systems.process.call_args_list[0].args == (
            (characteristics, AgentLifeCyclePhase.PRE_STATE_CHANGE, [])
        )
        assert agent.internal_systems.process.call_args_list[1].args == (
            (characteristics, AgentLifeCyclePhase.POST_STATE_CHANGE, [])
        )
