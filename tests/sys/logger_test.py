...
from agents_playground.sys.logger import LOG_TABLE_NONUNIFORM_ERR, LogTableError, TableStats, build_table_format, determine_table_stats, format_table_rows, stringify
...
class TestLogTable:
    def test_uniform_table(self, uniform_table) -> None:
        stats: TableStats = determine_table_stats(uniform_table)
    ...
    def test_nonuniform_table(self, nonuniform_table: list[list]) -> None:
        with pytest.raises(LogTableError) as e:
            determine_table_stats(nonuniform_table)
        assert LOG_TABLE_NONUNIFORM_ERR in str(e.value)

    def test_build_table(self, assorted_table) -> None:
        separator = " | "
        rows_of_strings: list[list[str]] = stringify(assorted_table)
        header = ["A", "B", "C", "D"]
        ...
        formatted_rows: list[str] = format_table_rows(table_stats, formatter, header, rows_of_strings)
        print("\n".join(formatted_rows))
        assert False