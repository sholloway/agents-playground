...
class TestSimLoop:
    ...
    def test_process_sim_cycle(self, mocker: MockFixture) -> None:
        scheduler = SimpleNamespace(
            queue_holding_tasks=mocker.Mock(), consume=mocker.Mock()
        )
        context = mocker.Mock()
    ...
    def test_update_renderer(self, mocker: MockFixture) -> None:
        ...
        scene = Scene()
        scene.add_entity(
            "fake_entity", SimpleNamespace(toml_id=1, update=mocker.Mock())
        )
        scene.add_entity(
            "fake_entity", SimpleNamespace(toml_id=2, update=mocker.Mock())
        )
        scene.add_entity(
            "fake_entity", SimpleNamespace(toml_id=3, update=mocker.Mock())
        )
