...
class TestCounter:
  def test_counter_increment(self) -> None:
    count_up = CounterBuilder.integer_counter_with_defaults(start = 3, increment_step=2)
  ...
  def test_counter_decrement(self) -> None:
    count_down = CounterBuilder.integer_counter_with_defaults(start = 15, decrement_step=5)
  ...
  def test_constrained_decrement(self) -> None:
    count_down = CounterBuilder.integer_counter_with_defaults(start = 10, decrement_step=1, min_value = 7)
  ...
  def test_min_value_check(self) -> None:
    count_down = CounterBuilder.integer_counter_with_defaults(start = 1004, decrement_step=1, min_value = 1000)
  ...
  def test_constrained_increment(self) -> None:
    count_up = CounterBuilder.integer_counter_with_defaults(start = -4, decrement_step=1, max_value = 0)
