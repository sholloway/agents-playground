...
def plot_benchmarks(metrics: Dict):
    ...
    finished_tasks = filter(
        lambda m: m.complete() == True, metrics["task_times"].values()
    )
    time_to_start, time_to_process, time_to_complete = zip(
        *map(find_task_metric_deltas, finished_tasks)
    )
    ...
    p90_time_to_start = round(
        quantiles(sorted(time_to_start), n=10, method="inclusive")[8], 3
    )
    p90_processing_time = round(
        quantiles(sorted(time_to_process), n=10, method="inclusive")[8], 3
    )
    p90_time_to_completion = round(
        quantiles(sorted(time_to_complete), n=10, method="inclusive")[8], 3
    )
    ...
    for task_id, task_metric in metrics["task_times"].items():
        ...
        x = [
            task_metric.registered_time,
            task_metric.started_time,
            task_metric.completed_time,
            task_metric.removed_time,
        ]
...
if __name__ == "__main__":
    ...
    schedule_thread = threading.Thread(
        name="schedule_thread", target=ts.consume, args=(), daemon=False
    )
